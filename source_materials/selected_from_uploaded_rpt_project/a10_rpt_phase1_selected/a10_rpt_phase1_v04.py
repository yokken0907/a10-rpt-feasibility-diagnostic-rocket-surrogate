#!/usr/bin/env python3
"""
A10-RPT Phase 1 v0.4
Mission-variable-preserving structured-prior control
for a nondimensional rocket-combustion-stability surrogate.

v0.4 changes:
- Keeps v0.1, v0.2, and v0.3 A10 baselines.
- Adds thermal-polished A10 controllers that preserve the successful v0.3 supply barrier.
- Adds a late-burn thermal barrier and anticipatory cooling using h, a^2, p, and remaining impulse margin.
- Tests whether the remaining v0.3 failures are thermal-tail failures rather than supply-margin failures.
- Separates mission-feasibility diagnostics from aggregate loss and reports failure reasons.
...
def controller_a10_thermal_v04(x, tau, hist, dt, p, alpha=None):
    """
    v0.4 A10-RPT: thermal-polished supply-barrier controller.

    Diagnostic motivation:
    v0.3 solved the migrated supply-margin bottleneck, but the residual failures
    were dominated by thermal-load excursions. v0.4 keeps the v0.3 architecture
    and adds a separate thermal barrier rather than folding thermal risk into a
    monolithic gate.

    Design idea:
      1. Retain the supply barrier that kept fail_m_rate at zero in v0.3.
      2. Spend part of the surplus impulse margin to reduce late-burn heat.
      3. Increase cooling/damping authority preemptively when h, a^2, or sustained p
         indicate future heat accumulation.
    """
    pc, a, h, phi, m, I = x
    da = estimate_da(hist, dt)
    sigma_hat = p.sigma0 + p.sigma_p * (pc - p.p_s) + p.sigma_phi * phi
    ...
    # Main danger gate: still excludes supply margin, which has its own barrier.
    D_main = float(max(pc, a, h, abs(phi), impulse_lag / (tau + 0.05)))
    g = sigmoid(8.0 * (D_main - 0.76))

    # Late-burn thermal gate. It turns on before h reaches the hard threshold.
    late = sigmoid(12.0 * (tau - 0.58))
    thermal_risk = sigmoid(18.0 * (h - 0.82))
    predictive_heat = sigmoid(10.0 * ((0.55 * h + 0.25 * pc + 0.20 * a * a) - 0.72))
    gT = max(float(thermal_risk), float(late * predictive_heat))

    # Slightly more conservative nominal power than v0.3, with stronger nominal cooling.
    u = np.array([0.665, 0.0, 0.305, 0.0], dtype=float)

    # Power correction: thermal risk is allowed to spend impulse surplus, but not
    # to create impulse failure. The positive impulse_lag term remains conservative.
    duP = (
        -0.15 * pos(h - p.h_s)
        -0.10 * pos(pc - p.p_s)
        -0.07 * pos(a - p.a_s)
        +0.10 * impulse_lag
        -0.10 * gT * (0.4 + impulse_surplus)
    )

    # Keep phase weak; it is supply-expensive and can also carry heat cost through |phi|.
    duPhi = (
        -0.14 * a
        -0.004 * da
        -0.34 * phi
    ...
    # Stronger anticipatory cooling than v0.3. The late thermal term is the main
    # v0.4 change, but it is rate-limited by the same actuator model.
    duC = (
        +0.95 * pos(h - 0.64)
        +0.24 * a * a
        +0.13 * pos(pc - p.p_s)
        +0.22 * gT
        +0.08 * late * pos(pc - 0.80)
    )

    duD = (
        +0.98 * a
        +0.014 * pos(da)
        +0.26 * pos(sigma_hat) * a
        +0.06 * gT * a
    )

    u = u + g * np.array([duP, duPhi, duC, duD], dtype=float)

    # Supply barrier inherited from v0.3, slightly relaxed only when m is ample.
    m_risk = sigmoid(24.0 * (0.34 - m))
    uP_cap = 0.52 + 0.36 * sigmoid(14.0 * (m - 0.23))
    u[0] = min(u[0], uP_cap)
    u[1] *= (1.0 - 0.88 * m_risk)

    # Late thermal cap: if heat risk rises late in the burn and impulse is not lagging,
    # cap power gently. This is what should reduce fail_h_rate.
    if tau > 0.62 and h > 0.86 and I > 0.94 * tau:
        u[0] = min(u[0], 0.66 - 0.28 * min(h - 0.86, 0.35))
    return u


def controller_a10_thermal_refined_v04(x, tau, hist, dt, p, alpha=None):
    """v0.4 thermal-polished A10 with very narrow in-sector impulse recovery."""
    u = controller_a10_thermal_v04(x, tau, hist, dt, p, alpha)
    pc, a, h, phi, m, I = x
    safety_margin = max(pc, a, h, abs(phi), p.m_min / max(m, 1e-6))

    # Recover impulse only when the system is comfortably safe, cool, and supply-rich.
    if tau > 0.50 and safety_margin < 0.80 and h < 0.82 and m > 0.48 and I < tau:
        u[0] += 0.030 * (tau - I)

    # Add cooling, not power, when late thermal state approaches the boundary.
    if tau > 0.65 and h > 0.78:
        u[2] += 0.12 * pos(h - 0.78)
        u[3] += 0.04 * a
    return u
...
CONTROLLERS: Dict[str, Callable] = {
    ...
    "a10_thermal_v04": controller_a10_thermal_v04,
    "a10_thermal_refined_v04": controller_a10_thermal_refined_v04,
}
...
def save_trajectory_plots(outdir: str, base: Params, n_steps: int, alpha: np.ndarray | None = None) -> None:
    seed = 20260424
    scenario = sample_scenario(seed, base)
    selected = ["no_control", "power_only", "pid_damping", "lqr_like", "a10_v01", "a10_barrier_v03", "a10_thermal_v04", "a10_thermal_refined_v04"]
    ...
    out = trajectories["a10_thermal_refined_v04"]
    control_names = ["uP", "uPhi", "uC", "uD"]
    for idx, cname in enumerate(control_names):
        plt.figure(figsize=(9, 5))
        plt.plot(out["T"], out["U"][:, idx], label="a10_thermal_refined_v04")
        plt.xlabel("nondimensional burn time tau")
        plt.ylabel(cname)
