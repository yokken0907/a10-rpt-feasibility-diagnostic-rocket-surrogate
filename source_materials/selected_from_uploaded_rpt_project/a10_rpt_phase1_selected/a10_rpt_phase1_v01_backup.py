...
def controller_a10_refined(x, tau, hist, dt, p, alpha=None):
    """A10 + small in-sector refinement. Still constrained and conservative."""
...
CONTROLLERS: Dict[str, Callable] = {
    ...
    "a10_rpt": controller_a10,
    "a10_refined": controller_a10_refined,
}
...
def random_search_a10(
    ...
    _, best_summary = evaluate_controller("a10_train", controller_a10, train_seeds, base, n_steps, best_alpha)
    ...
    for i in range(n_iter):
        ...
        _, summary = evaluate_controller("a10_train", controller_a10, train_seeds, base, n_steps, cand)
...
def save_trajectory_plots(outdir: str, base: Params, n_steps: int, alpha: np.ndarray | None = None) -> None:
    seed = 20260424
    scenario = sample_scenario(seed, base)
    selected = ["no_control", "power_only", "pid_damping", "lqr_like", "a10_rpt", "a10_refined"]
    ...
    out = trajectories["a10_refined"]
    control_names = ["uP", "uPhi", "uC", "uD"]
    for idx, cname in enumerate(control_names):
        plt.figure(figsize=(9, 5))
        plt.plot(out["T"], out["U"][:, idx], label="a10_refined")
        plt.xlabel("nondimensional burn time tau")
        plt.ylabel(cname)
...
def main() -> None:
    ...
    summary_df = pd.DataFrame(summaries).sort_values("cvar90_L")
    ...
    print("\n=== Summary sorted by CVaR90 loss ===")
    cols = [
        "controller", "mean_L", "cvar90_L", "worst_L", "failure_rate",
        "mean_unsafe_duration", "mean_final_I", "worst_max_a", "worst_max_h", "worst_max_p", "worst_min_m"
    ]
